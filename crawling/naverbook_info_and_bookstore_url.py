...
def get_info():
    ...
    with open(book_info_file_name, "w", encoding = "utf-8-sig") as f:
        f.write("main_category,sub_category,book_id,book_title,book_author,publisher,publish_date,aladin,kyobo,yes24,interpark\n")
            ...
            for j in range(1,6): # 20개씩 5페이지로 구성되어있다 ## DEMO 5개씩만 가져옴
                ...
                for detail_a in detail_a_list:
                    ...
                    try:
                        ...
                        publisher = detail_soup.select_one("#container > div.spot > div.book_info > div.book_info_inner > div:nth-child(2)").find('a', class_='N=a:bil.publisher').string
                        book_info = driver.find_element(By.XPATH, "//div[@class='book_info_inner']/div[2]").text
                        bar_index = book_info.index('|')
                        book_author = book_info[3:bar_index]
                        book_author = book_author.replace(',', '')
                        right_bar_index = book_info.rfind('|')
                        publish_date = book_info[right_bar_index+1:]
                        
                        img_file = urllib.request.urlopen(book_cover).read()
                        print()
                        print(book_id,book_title,book_cover, book_author, publisher, publish_date)
                        ...
                        f.write(main_category+","+sub_category+","+str(book_id)+","+book_title+","+book_author+","+publisher+","+publish_date+","+bookstore_url_dict[bookstore[0]]+","+bookstore_url_dict[bookstore[1]]+","+bookstore_url_dict[bookstore[2]]+","+bookstore_url_dict[bookstore[3]]+"\n")
                        print("완료!")
